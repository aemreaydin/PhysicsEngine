...
namespace glm
{
	...
	inline int intersect_ray_sphere(const vec3& point, const vec3& dir, const vec3& center, float radius, float& t, vec3& q)
	{
		vec3 m = point - center;
		float b = dot(m, dir);
	...
	inline int intersect_moving_sphere_sphere(
		const vec3& c0, float r0, const vec3& v0,
		const vec3& c1, float r1, const vec3& v1, float& t)
	{
		vec3 s = c1 - c0;  // sphere center separation
		vec3 v = v1 - v0;  // relative motion of sphere 1 w.r.t. stationary s0
